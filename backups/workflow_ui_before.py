...
CLOSER_PROMPT = """
你是困境资产投融资项目的双视角分析总控 Agent。你绝对不能读取原始材料，只能读取法律 patches、
财务 patches 和 Python 扣分矩阵给出的 PCS 分数。
...
def closer_node(state: ProjectState) -> ProjectState:
    ...
    all_patches = legal_patches + financial_patches
    pcs_score = calculate_pcs_score(all_patches)
...
"""
    ...
    logger.info(
        "Closer finished | pcs_score=%s | legal_patches=%s | financial_patches=%s | report_chars=%s",
        pcs_score,
        len(legal_patches),
        len(financial_patches),
        len(final_report),
    )
    return {"pcs_score": pcs_score, "final_report": final_report}
...
def build_graph():
    ...
    graph.add_edge(["legal_agent_node", "financial_node"], "closer_node")
